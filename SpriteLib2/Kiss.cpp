...
Kiss::Kiss(string n, float ms, float pX, float pY, int fSX, int fSY, float cd, float ft, int hp, int rng, int dmg, bool R)
...
void Kiss::behave(Player &p1, vector<Enemy*> &enemies, vector<Sprite*>& enemySprite, vector<Projectile_E*> &pro, vector<Sprite*>& proSprite)
{
	...
	if (dummy == false && flinching == false && knockback == false)
	{
		...
		if (attacking == false && attack_once == true) // End Attack
		{
			for (int i = 0; i < enemies.size(); i++)
			{
				if (*enemies[i] == &(*this) && enemySprite[i]->get_current_frame() == 5)
				{
					enemySprite[i]->set_animation("idle");
					attack_once = false;
				}
